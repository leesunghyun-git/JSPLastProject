...
public class ChatManager {
	...
	public void onOpen(Session session,EndpointConfig config) throws Exception{
		...
		while(it.hasNext())
		{
			...
			{
				ss.getBasicRemote().sendText("msg:[🙏 알림]"+vo.getName()+"님이 입장 하셨습니다.");
			}
		}
		System.out.println("클라이언트 접속:"+vo.getId()+","+vo.getName());
	}
	// 접속 해제 처리 =>
	@OnClose
	public void onClose(Session session) throws Exception
	{
		...
		while(it.hasNext())
		{
			...
			if(ss.getId()!=session.getId()) // 본인이 아닌 경우
			{
				ss.getBasicRemote().sendText("msg:[🙏 알림]"+vo.getName()+"님이 퇴장 하셨습니다.");
			}
		}
